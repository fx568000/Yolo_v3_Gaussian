...
def y_pred_graph(raw_y_pred, anchors, input_shape):
    ...
    raw_y_pred = K.reshape(yolo_output, [-1, grid_shape[0], grid_shape[1], 3, 14])
    Args:
    ...
    grid_shape = K.shape(raw_y_pred)[1:3]# x,y
    grid_y = K.tile(K.reshape(K.arange(0, stop=grid_shape[0]), [-1, 1, 1, 1]), [1, grid_shape[1], 1, 1])
    grid_x = K.tile(K.reshape(K.arange(0, stop=grid_shape[1]), [1, -1, 1, 1]), [grid_shape[0], 1, 1, 1])
    grid = K.concatenate([grid_x, grid_y], axis=-1) ##### 连接两个数组,默认为-1，合并成一整行
    grid = K.cast(grid, K.dtype(raw_y_pred))##### [?,?,?,3,14] # 格式转换为raw_y_pred型
