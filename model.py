...
def yolo_body(anchors, num_classes=20, score_threshold=0.01):
    ...
    fm_52_input = Input(shape=(None, None, num_anchors_per_layer, num_classes + 5), name='fm_52_input') # 类似于*y_true
    image_shape_input = Input(shape=(2,), name='image_shape_input')
    darknet = Model([image_input], darknet_body(image_input)) # 原版darknet = Model(inputs, darknet_body(inputs))
    ...
    loss = Lambda(yolo_loss,
                  ...
                  arguments={'anchors': anchors,
                             ...
                             'ignore_thresh': 0.5})(
        [y1, y2, y3, fm_13_input, fm_26_input, fm_52_input]) # 类似原版keras:[*model_body.output, *y_true]
    training_model = Model([image_input, fm_13_input, fm_26_input, fm_52_input], loss, name='yolo') # 函数式模型接口：model = Model(inputs=[a1, a2], outputs=[b1, b3, b3])
